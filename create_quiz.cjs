...
const postData = JSON.stringify({
  ...
  properties: {
    "標題 (Title)": { title: [{ text: { content: "[測驗企劃] 大腦當機指數評測：8題問卷與權重分析" } }] },
  ...
  children: [
    {
      object: "block",
      type: "paragraph",
      paragraph: { rich_text: [{ text: { content: "這份測驗的底層邏輯是透過 8 個日常情境，精準測出用戶的「腦波失衡狀態」，並將其導向我們對應的 4 款音頻產品。" } }] }
    },
    {
      object: "block",
      type: "heading_3",
    ...
    },
    {
      ...
      bulleted_list_item: { rich_text: [{ text: { content: "[A] 腦壓過高 (Theta波缺乏) ➡ 導向：腦壓釋放・歸零頻率" } }] }
    },
    {
      object: "block",
      type: "bulleted_list_item",
      bulleted_list_item: { rich_text: [{ text: { content: "[B] 淺眠難入睡 (Delta波缺乏) ➡ 導向：深度睡眠・修復頻率" } }] }
    },
    {
      object: "block",
      type: "bulleted_list_item",
      bulleted_list_item: { rich_text: [{ text: { content: "[C] 頻寬受限/易分心 (Beta波紊亂) ➡ 導向：深度心流・工作頻率" } }] }
    },
    {
      object: "block",
      type: "bulleted_list_item",
      bulleted_list_item: { rich_text: [{ text: { content: "[D] 匱乏防衛機制 (Solfeggio/頻率不對齊) ➡ 導向：財富豐盛・顯化頻率" } }] }
    },
    {
      object: "block",
      type: "heading_2",
    ...
    },
    {
      object: "block",
      type: "paragraph",
      paragraph: { rich_text: [{ text: { content: "【多數 A】過載運轉的超級電腦 (對應：腦壓釋放音頻)\n分析：大腦快取記憶體已滿，Beta波持續高頻震盪。你需要Theta波的深層清洗，像降溫系統一樣釋放腦壓。" } }] }
    },
    {
      object: "block",
      type: "paragraph",
      paragraph: { rich_text: [{ text: { content: "【多數 B】無法休眠的伺服器 (對應：深度睡眠音頻)\n分析：即使身體躺平，神經系統依然處於備戰狀態。你需要一個強制的「頻率關機鍵」，強迫大腦進入Delta頻率。" } }] }
    },
    {
      object: "block",
      type: "paragraph",
      paragraph: { rich_text: [{ text: { content: "【多數 C】頻寬受限的處理器 (對應：極致專注音頻)\n分析：注意力被各種碎片化資訊切割。你需要Alpha/Low Beta波來建立「防干擾結界」，穩固思緒。" } }] }
    },
    {
      object: "block",
      type: "paragraph",
      paragraph: { rich_text: [{ text: { content: "【多數 D】防火牆過高的封閉網路 (對應：財富顯化音頻)\n分析：潛意識對外在世界充滿防衛與匱乏感。你需要特定的Solfeggio頻率，清理匱乏代碼，重新校準吸引力磁場。" } }] }
    }
  ]
});
